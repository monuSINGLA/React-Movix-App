...
const DetailsBanner = ({ video, crew }) => {
    ...
    const[videoId, settVideoId] = useState(null)
    ...
    const writer = crew?.filter(r=> r.job === "Writer" || r.job === "Screenplay" || r.job === "Story")
    

    const runTimeFormate = (totalMinutes)=>{
        const hours = Math.floor(totalMinutes / 60);
    const minutes = totalMinutes % 60;

    return `${hours>0 ?`${hours}`:""} ${hours>0 ? `${hours >1 ? 'hours' : 'hour'}` : ""} ${minutes > 0 ? `${minutes}`:""  } ${minutes > 0 ? `${minutes>1 ? "minutes": "minute"}`: ""}`;
    }
